...
It migrates configuration, the generated workflow, and machine-owned registry
...
It explains store prerequisites, `smf init`, the generated GitHub Actions
workflow, safe candidate-only defaults, candidate testing, and recovery. The
Flutter application does not add `smf_cli` to `dev_dependencies`.
