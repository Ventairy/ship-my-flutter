...
void main() {
  ...
  test('publishes only operation-specific package entrypoints', () async {
    ...
    expect(pubspec, isNot(contains('executables')));
    expect(await File('bin/smf.dart').exists(), isFalse);
    for (final name in const <String>[
      ...
      expect(await File('bin/$name.dart').exists(), isTrue, reason: name);
    }
  });

  test('project-local executables initialize, validate, and plan', () async {
  ...
  test(
    ...
    () async {
      ...
      expect(output.single, contains('Usage: dart run smf:init [options]'));
