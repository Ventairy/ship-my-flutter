...
- `Ventairy/ship-my-flutter` contains the CLI, schemas, release planner, GitHub orchestration, signing implementation, and App Store Connect client.
- `Ventairy/ship-my-flutter-action` vendors a built copy of the core and exposes the three workflow phases as a single GitHub Action.

The action bundle is self-contained. A consumer does not install Node dependencies or Fastlane.
...
6. enforces the tracked Dart lockfile, then runs `flutter build ipa --no-pub` with independent iOS version/build values.
